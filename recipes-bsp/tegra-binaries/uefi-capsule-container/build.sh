...
case "${DEVICE_TYPE}" in
	"edgeai-orn-nx" | "edgeai-orn-nano")
		...
		install -m 0644 /build_dir/edgeai-orn/tegra234-edgeai-orn-gpio-default.dtsi "${bct_dir}/"
		install -m 0644 /build_dir/edgeai-orn/tegra234-edgeai-orn-padvoltage-default.dtsi "${bct_dir}/"
		install -m 0644 /build_dir/edgeai-orn/tegra234-edgeai-orn-pinmux.dtsi "${bct_dir}/"
		install -m 0644 /build_dir/edgeai-orn/tegra234-edgeai-orn-mb2-bct-misc-p3767-0000.dts "${bct_dir}/"
		...
		install -m 0644 /build_dir/edgeai-orn/dtbs/*.dtb kernel/dtb/
...
EOF
	...
	"jetson-orin-nano-seeed-j3010" | "jetson-orin-nx-seeed-j4012")
		...
		# The r39.2.0 branch is often overwritten in upstream,
		# breaking builds for this package. We thus checkout the
		# L4T 39.2.0 branch  at the current HEAD revision
		git checkout r39.2.0
		popd
		popd
...
# The BUP wrapper can succeed even when all board configurations fail.
...
test -s "${payload_path}" || { echo "ERROR: missing BUP payload" >&2; exit 1; }
sudo ./generate_capsule/l4t_generate_soc_capsule.sh -i "${payload_path}" -o ./TEGRA_BL.Cap t234
test -s TEGRA_BL.Cap || { echo "ERROR: missing UEFI capsule" >&2; exit 1; }
